...
pub const WW : usize =  128;
pub const HH : usize =  160;
...
pub const ST7735 : &[u8] = &[
    ...
    //
    ST7735_CASET,   4, (XOFFSET >> 8) as u8, ((XOFFSET) & 0xff) as u8, ((WW + XOFFSET - 1) >> 8) as u8,
    ((WW + XOFFSET - 1) & 0xff ) as u8, // Column addr set: XSTART=0, XEND=width
    ST7735_RASET,   4, (YOFFSET >> 8) as u8, ((YOFFSET) & 0xff) as u8, ((HH + YOFFSET - 1) >> 8) as u8,
    ((HH + YOFFSET - 1) & 0xff) as u8, // Row addr set: YSTART=0, YEND=height
    //
    ST7735_MADCTL,  1, 0x78, //  0x28 <-inverted->:   0x78: <-normal->, || 48 , || 18
    ST7735_DISPON,  0, 
    ST7735_SLPOUT,  0,
    0
