...
impl<'a> Ili9341<'a> {
        ...
        for c in text.chars() {
            width += match c {
                ...
                x => {
                    ...
                    return self.current_font().font.glyphs[(c as usize) - (first as usize)]
                        .x_advance as usize;
                }
            }
        }
    ...
    pub fn write_char(&mut self, c: char) -> () {
        ...
        self.cursor_x += self.my_draw_char(self.cursor_x, self.cursor_y, c, self.fg, self.bg);
    }
    ///
    ///
    ///
    ...
    fn my_draw_char(&mut self, x: usize, y: usize, c: usize, fg: u16, bg: u16) -> usize {
        ...
        let top: usize =
            (self.current_font().max_height as isize + glyph.y_offset as isize) as usize;
        // Special case
        if full_c == (' ' as usize) {
            if y >= top {
                self.my_square(
                    ...
                    (y - top) as usize,
                    advv, //Fix!
                    self.current_font().max_height + 2,
                    bg,
                );
            }
            return advv;
        ...
        if y > self.current_font().max_height {
            self.my_square(x, y - self.current_font().max_height, advv, top, bg);
        }
        let bottom: isize = -(glyph.y_offset as isize) - (h as isize);
        if bottom >= -2 {
            self.my_square(
                x,
                ((y as isize) - bottom) as usize,
                advv,
                (bottom + 2) as usize,
                bg,
            );
